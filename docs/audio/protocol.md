...
* Android audio endpoint: UDP `40125` (`NX01`, feedback, settings, time sync,
  heartbeat, connection control and Android peer calibration control).
* Desktop control endpoint: UDP `40126` (`NXCR` requests, responses and bye).
* mDNS `_nexus._udp.local.` advertises the endpoint port and a stable
...
`NXCR` request/response/bye packets are retransmitted every 1500 ms for at
...
`NXHB` is a 32-byte packet: magic (4), version (1), kind (1), reserved (2),
...
`TimestampNs` fields and `NXTS` exchange fields always use each process's
...
`NX01` audio packets carry a session and sequence. Receivers discard packets
...
`NXAC` datagrams are exchanged directly between two Android receivers on UDP
`40125`; the desktop never receives or relays them. The requester must first
receive explicit peer confirmation, then performs `NXTS` probes and sends each
