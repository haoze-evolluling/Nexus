...
本文档用于记录 SyncTouch 项目全新 Android 正式发布签名证书的技术规格、安全管理规范、离线私有保管流程以及多环境配置指引。
...
- **凭据与代码解耦**：密钥文件和凭据严格置于本地（`SyncTouch-keystore/`），仅提交脱敏模版 `keystore.properties.example`，避免敏感凭据再次入库。
...
| 参数项 | 取值 |
| :--- | :--- |
| **密钥库文件** | `SyncTouch-keystore/synctouch-release.jks`（本地私有，不入库） |
| **密钥库格式** | `PKCS12`（业界标准格式） |
| **签名别名 (Alias)** | `synctouch-release` |
| **密钥算法与长度** | `RSA` / `4096 bits` |
| **签名算法** | `SHA384withRSA` |
| **证书所有人 (DName)** | `CN=SyncTouch, OU=SyncTouch Team, O=SyncTouch, C=CN` |
...
1. **创建专用保管目录**：
   ...
   `My Drive/Private-Credentials/SyncTouch-Android-Signing/`
2. **上传必要凭据（推荐直接上传整个 `SyncTouch-keystore` 文件夹）**：
   本地所有的签名私密文件均统一存放在 `SyncTouch-keystore/` 目录下（受 `.gitignore` 保护不入库）：
   - `SyncTouch-keystore/synctouch-release.jks`（密钥库文件）
   - `SyncTouch-keystore/keystore.properties`（签名密码与别名配置）
   您可以直接将整个 `SyncTouch-keystore` 目录（或压缩包）上传至 Google Drive 专用目录中。
3. **安全权限控制**：
...
### 步骤 1：获取密钥
1. 从个人的私有 Google Drive 目录中下载 `SyncTouch-keystore` 文件夹（包含 `synctouch-release.jks` 和 `keystore.properties`）。
2. 放置到项目根目录下：
   ```text
   SyncTouch/
   └── SyncTouch-keystore/
       ├── synctouch-release.jks
       └── keystore.properties
   ```
   > 注：若之前仅备份了密钥文件与密码记录，也可复制根目录下的 `keystore.properties.example` 至 `SyncTouch-keystore/keystore.properties`，填入实际密码即可。

### 步骤 2：编译与打包构建选项
- **使用构建脚本（推荐）**：
  直接运行项目根目录下的 `build_apk.bat`，菜单提供以下选项：
  - `[1] Debug`：开发调试版，使用本地 Android SDK 默认调试密钥。
  - `[2] Release`：正式发布包，开启 R8 混淆优化，使用 `SyncTouch-keystore` 正式证书签名。
  - `[3] Debug (Release Key)`：开发调试版，快速编译且可断点调试，但统一使用 `SyncTouch-keystore` 正式证书签名（支持直接覆盖安装测试，无需卸载已有的正式版）。
...
- **构建产物位置**：
  ...
  - 带版本号命名输出：`app/build/outputs/apk/versioned/release/SyncTouch-release-v<version>.apk`
...
- `Signer #1 certificate DN` 明确显示 `CN=SyncTouch, OU=SyncTouch Team, O=SyncTouch, C=CN`。
