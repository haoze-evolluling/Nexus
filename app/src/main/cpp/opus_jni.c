...
JNIEXPORT jlong JNICALL Java_com_haoze_claudekeyboard_audio_OpusNative_createEncoder(JNIEnv *env, jclass cls, jint rate, jint channels, jint bitrate) {
    (void)env; (void)cls;
    int err = OPUS_OK;
...
JNIEXPORT jbyteArray JNICALL Java_com_haoze_claudekeyboard_audio_OpusNative_encode(JNIEnv *env, jclass cls, jlong handle, jbyteArray pcm, jint frameSamples) {
    (void)cls;
...
JNIEXPORT jboolean JNICALL Java_com_haoze_claudekeyboard_audio_OpusNative_setEncoderBitrate(JNIEnv *env, jclass cls, jlong handle, jint bitrate) { (void)env; (void)cls; return as_encoder(handle) && opus_encoder_ctl(as_encoder(handle), OPUS_SET_BITRATE(bitrate)) == OPUS_OK; }
JNIEXPORT void JNICALL Java_com_haoze_claudekeyboard_audio_OpusNative_destroyEncoder(JNIEnv *env, jclass cls, jlong handle) { (void)env; (void)cls; if (handle) opus_encoder_destroy(as_encoder(handle)); }

JNIEXPORT jlong JNICALL Java_com_haoze_claudekeyboard_audio_OpusNative_createDecoder(JNIEnv *env, jclass cls, jint rate, jint channels) { (void)env; (void)cls; int err=OPUS_OK; OpusDecoder *d=opus_decoder_create(rate, channels, &err); return (d && err==OPUS_OK) ? (jlong)(intptr_t)d : 0; }
