...
class KeyboardSender(
    ...
    // Held-key state for press/release style sending (6-key rollover, Bluke semantics)
    ...
    private fun sendHeldKeysReportLocal() {
        val report = KeyboardReport()
        report.bytes[1] = activeModifiers.toByte()
        activeKeys.copyInto(report.bytes, 3)
        if (!hidDevice.sendReport(host, KeyboardReport.ID, report.bytes.copyOfRange(1, report.bytes.size))) {
            Log.e(TAG, "Report wasn't sent")
            onSendError?.invoke("Keyboard report send failed")
        }
    ...
    /**
     * Bluke-style press/release sending: tracks held modifiers (0xE0-0xE7) and up to
     * six standard keys, emitting the full current report on every transition.
     */
    @Synchronized
    fun sendKey(keyCode: Int, isPress: Boolean) {
        when {
            keyCode in 0xE0..0xE7 -> {
            ...
            keyCode != 0 -> {
                val heldIndex = activeKeys.indexOf(keyCode.toByte())
                if (isPress) {
                    if (heldIndex < 0) {
                        val emptySlot = activeKeys.indexOf(0.toByte())
                        if (emptySlot >= 0) activeKeys[emptySlot] = keyCode.toByte()
                    }
                } else if (heldIndex >= 0) {
                    activeKeys[heldIndex] = 0
                }
            }
            else -> return
        }
        sendHeldKeysReportLocal()
    }
